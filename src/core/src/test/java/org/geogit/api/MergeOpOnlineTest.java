...
public class MergeOpOnlineTest extends MultipleRepositoryTestCase {
    ...
    public void testOneCommitMerge() throws Exception {
         ...
         * MERGE - REBASE
         */
        client.merge().setComment("Merged remote, one commit merge").include(clientRemoteMaster)
                .call();
    ...
    public void testMergeCrissCrossTest() throws Exception {
        ...
        // merge the fetch
        client.merge().setComment("Merged remote, criss cross test 1").include(clientRemoteMaster)
                .call();
        ...
        // merge the fetch
        client.merge().setComment("Merged remote, criss cross test 2").include(clientRemoteMaster)
                .call();
    ...
    public void testMergeComplex() throws Exception {
        ...
        RevCommit serversLastCommit = server.commit().setMessage("commited a new feature")
                .setAll(true).call();
        this.server.getRepository().close();
    ...
    public void testMergeRemoteMasterConflict() throws Exception {
        ...
        server.commit().setMessage("server - commited a new point feature at point(1 1)")
                .setAll(true).call();
        this.server.getRepository().close();
        ...
        MergeResult mergeResults = client.merge().setMergeStrategy(new ReverseRebaseMergeOp())
                .setComment("Merged remote, conflicting features").include(clientRemoteMaster)
                .call();
        // for (DiffEntry diff : mergeResults.getDiffs()) {
        // System.out.println(diff);
        // }

        Feature featurePoints1 = client.getRepository().getFeature(points_conflicting1_1.getType(),
        ...
        this.client
                .commit()
                .setMessage(
                        "client - commited changed the feature Points.4 from point(1 1) to point(3 3)")
                .setAll(true).call();
        insert(this.client, points3_modify);
        this.client.commit().setMessage("client - commited some stuff Points.2").setAll(true)
                .call();
        insert(this.client, points3);
        this.client.commit().setMessage("client - commited some stuff Points.3").setAll(true)
                .call();
        ...
        RevCommit commit = this.server
                .commit()
                .setMessage(
                        "server - commited changed the feature Points.4 from point(2 2) to point(4 4)")
                .setAll(true).call();
        this.server.getRepository().close();
        ...
        featurePoints1 = client.getRepository().getFeature(points_conflicting3_3.getType(),
         ...
         * 'remotes/project0/master', is the ID of that the same as the HEAD ID of the server?
         */
        clientRemoteMaster = this.client.getRepository().getRef(
                Ref.REMOTES_PREFIX + "project0/" + Ref.MASTER);
        assertEquals(clientRemoteMaster.getObjectId(), this.server.getRepository().getHead()
        ...
        /**
         * 1. get a list of the features changed in the remote branch 2. check if they were also
         * changed in the master 3. if no - continue if yes - merge the change - server wins, for
         * the moment override the client change
         * 
         * ====================================================================
         * ====================================================
         */
        ...
        while (logs.hasNext()) {
            ...
            if (!MergeUtils.onBranch(this.client.getRepository().getHead().getObjectId(),
                    rc.getId(), this.client.getRepository())) {
        ...
        for (Feature serverFeature : serverFeatures) {
            for (Feature clientFeature : clientFeatures) {
                if (clientFeature.getIdentifier().equals(serverFeature.getIdentifier())) {
    ...
    private Collection<Feature> extractFeatures(final RevCommit rc, final Repository repo) {
            ...
            public boolean visitEntry(Ref ref) {
                if (ref.getType().equals(RevObject.TYPE.TREE)) {
                    ...
                    if (TYPE.BLOB.equals(ref.getType())) {
                        RevBlob revblob = repo.getObjectDatabase().getBlob(ref.getObjectId());
                        Feature f = repo.getFeature(pointsType, ref.getName(), revblob.getId());
