...
public class RepositoryTest extends RepositoryTestCase {
    ...
    public void testResolve() throws Exception {
        ...
        ObjectId commitId = repo.getObjectDatabase().put(getRepository().newCommitWriter(insert));
    ...
    public void testResolveType() throws Exception {
        ...
        repo.getObjectDatabase().put(treeId, getRepository().newRevTreeWriter(repo.newTree()));

        {
            RevCommit c = new RevCommit(ObjectId.NULL);
            c.setTreeId(ObjectId.NULL);
            repo.getObjectDatabase().put(commitId, getRepository().newCommitWriter(c));
        }
