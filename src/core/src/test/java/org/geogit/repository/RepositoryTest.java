...
public class RepositoryTest extends RepositoryTestCase {
    ...
    public void testResolve() throws Exception {
        ...
        ObjectId commitId = repo.getObjectDatabase().put(
                WrappedSerialisingFactory.getInstance().createCommitWriter(insert));
    ...
    public void testResolveType() throws Exception {
        ...
        repo.getObjectDatabase().put(treeId,
                WrappedSerialisingFactory.getInstance().createRevTreeWriter(repo.newTree()));

        {
            RevCommit c = new RevCommit(ObjectId.NULL);
            c.setTreeId(ObjectId.NULL);
            repo.getObjectDatabase().put(commitId,
                    WrappedSerialisingFactory.getInstance().createCommitWriter(c));
        }
