...
public class PayloadUtil {
    ...
    public FetchResult applyPayloadTo(String branchName, IPayload payload) throws Exception {
        ...
        for (RevCommit commit : payload.getCommitUpdates()) {
            /*
             * ignore anything that is already in the repository - its likely this situation is a
             * commit that has been committed by this repository and pushed back to the server - and
        ...
        /*
         * Update the local repos branch refs for the remote so the branch head is pointing at the
         * correct commit
         * 
         * for (RevTag tag: payload.getTagUpdates()) { deltas++; ObjectId tagId =
         * objectInserter.insert(new RevTagWriter(tag)); Ref ref = new Ref(remote.getName(), tagId,
         * TYPE.TAG); getRepository().getRefDatabase().put(ref); }
         */
