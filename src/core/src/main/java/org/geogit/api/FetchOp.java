 ...
 * HEAD of remote.
 * 
 * http://git-scm.com/gitserver.txt - This document doesn't seem to be available anymore at this
 * location.
 * 
 * @author jhudson
 */
public class FetchOp extends AbstractGeoGitOp<FetchResult> {
     ...
     * res/remotes/REF_NAME/REMOTE_ID send remote_service (REF_NAMES<REMOTE_ID> receive pack file
     * with 1. REMOTE_NAME <commits> 2. Add commits to object DB 3. update ref in ref DB 4. write
     * config
     */
    @Override
    public FetchResult call() throws Exception {
        /*
         * The Result object holds a counter on all the operations that occur on a fetch this will
         * be used by a pull operation to identify if it needs to do a merge.
         */
        ...
        for (RemoteConfigObject remote : remotes.values()) {
            if (remote != null) { /* can never be too careful */
                ...
                IRemote remoteRepo = RemoteRepositoryFactory
                        .createRemoteRepositroy(remote.getUrl());
                 ...
                 */
                IPayload payload = remoteRepo.requestFetchPayload(RefIO.getRemoteList(
                        getRepository().getRepositoryHome(), remote.getName()));
