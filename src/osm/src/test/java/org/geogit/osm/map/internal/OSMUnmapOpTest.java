...
public class OSMUnmapOpTest extends RepositoryTestCase {
    ...
    public void testMappingAndUnmappingOfWays() throws Exception {
        ...
        ArrayList<Coordinate> coords = Lists.newArrayList(((Geometry) values.get(0).get())
                .getCoordinates());
        coords.add(new Coordinate(0, 1));
        assertEquals(31347480l, values.get(1).get());
        ...
        assertEquals(
                "LINESTRING (7.1960069 50.7399033, 7.195868 50.7399081, 7.1950788 50.739912, 7.1949262 50.7399053, "
                        ...
                        + "7.1924199 50.7397781, 0 1)", values.get(0).get().toString());
        assertEquals(31347480l, ((Long) values.get(1).get()).longValue());
        assertEquals("newname", values.get(2).get().toString());
        // unmap
        ...
        assertEquals("lit:yes|highway:residential|name:newname", values.get(2).get().toString());
        String nodes = values.get(1).get().toString();
        ...
        assertEquals("POINT (0 1)", values.get(1).get().toString());
    ...
    public void testMappingAndUnmappingOfNodes() throws Exception {
        ...
        assertEquals(wkt, values.get(0).get().toString());
        assertEquals(507464799l, values.get(1).get());
        GeometryFactory gf = new GeometryFactory();
        SimpleFeatureBuilder fb = new SimpleFeatureBuilder((SimpleFeatureType) featureType.get()
                .type());
        ...
        assertTrue(mapped.isPresent());
        values = mapped.get().getValues();
        assertEquals("POINT (0 1)", values.get(0).get().toString());
        assertEquals(507464799l, ((Long) values.get(1).get()).longValue());
        assertEquals("newname", values.get(2).get().toString());
        ...
        assertEquals("14220478", values.get(0).get().toString());
        assertEquals("1355097351000", values.get(3).get().toString());
        assertEquals("2", values.get(5).get().toString());
